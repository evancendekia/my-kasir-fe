...
export default class DetailMeja extends Component {
	...
	getcolor = (status, isBooked)=>{
		// switch(status){
		// case "A" :  return 'bg-success text-white';
		// 						break;
		// case "O" :  return 'bg-info text-white';
		// 						break;
      ...
      <Col md={3} className="mt-3 px-3">
        ...
        <hr />
          <Card className="hasil" style={{height: "80vh"}} >
            <Card.Header className={`fw-bold ${this.getcolor(selectedTable.status, selectedTable.isBooked)}`}>Meja : {selectedTable && selectedTable.nomor != null ? selectedTable.nomor : 'Pilih meja...'}</Card.Header>
